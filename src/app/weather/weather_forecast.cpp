...
bool weather_forecast_wifictl_event_cb(EventBits_t event, void *arg)
{
    ...
    case WIFICTL_CONNECT:
        weather_config_t *tmp_weather_config = weather_get_config();
...
void weather_forecast_sync_Task(void *pvParameters)
{
    ...
    log_i("start weather forecast task: Heap: %d, PSRAM: %d, uptime: %d", ESP.getFreeHeap(), ESP.getFreePsram(), millis() / 1000);
    ...
    log_i("finsh weather forecast task: Heap: %d, PSRAM: %d, uptime: %d", ESP.getFreeHeap(), ESP.getFreePsram(), millis() / 1000);
    vTaskDelete(NULL);
}
