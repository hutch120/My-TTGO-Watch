    ...
    void onWrite(BLECharacteristic *pCharacteristic)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < pCharacteristic->getValue().length(); i++)
            {
                ...
                {
                case EndofText:
                    blectl_delete_gadgetbridge_msg();
                ...
                case DataLinkEscape:
                    ...
                    break;
                case LineFeed:
                    log_i("message complete, fire BLTCTL_MSG callback");
                    ...
                    break;
                default:
                    blectl_add_char_to_gadgetbridge_msg(msg[i]);
                }
            }
            free(msg);
        }
...
void blectl_send_msg(char *msg)
{
    ...
    {
        log_e("msg can't send while bluetooth is not connected %s", msg);
    }
#endif
}
