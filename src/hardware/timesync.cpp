...
void timesync_Task(void *pvParameters)
{
    log_i("time sync start: Heap: %d, PSRAM: %d, uptime: %d", ESP.getFreeHeap(), ESP.getFreePsram(), millis() / 1000);
    if (xEventGroupGetBits(time_event_handle) & TIME_SYNC_REQUEST)
    {
        struct tm info;
    ...
    log_i("time sync finish: Heap: %d, PSRAM: %d, uptime: %d", ESP.getFreeHeap(), ESP.getFreePsram(), millis() / 1000);
    vTaskDelete(NULL);
}
